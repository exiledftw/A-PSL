...
### What Was Done
- Built the "Virtual Webcam" pipeline for XR Animator: `xr_pipeline/stream_controller.py` streams reference `.mp4` videos to OBS Virtual Camera, driving the VRM avatar in real-time.
- Built the "NLP Trigger" system: `xr_pipeline/audio_listener.py` uses `SpeechRecognition` and `thefuzz` to transcribe doctor speech, match intents (e.g., "dizzy"), and trigger the `stream_controller.py` via local UDP sockets (Port 5005).
- Designed the full architecture bridging speech -> text -> NLP -> virtual cam -> XR animator -> SANA Avatar.

### What Needs to Be Done
- [ ] Record the remaining dataset videos (Khizer) to feed into the pipeline.
