# Item 1: Prove the Full Pipeline — ASL Foundation → PSL Medical MVP

> **Priority:** 🔥 Critical 
> **Status:** 🔴 Not Started 
> **Owner:** Khizer + Rehan 
> **End Goal:** A working two-way Medical PSL prototype (Patient→Doctor via sign recognition, Doctor→Patient via Voice→Avatar) 
...
```mermaid
graph TD
    subgraph S1["Strategy 1: ASL Foundation (THIS ITEM)"]
        A["Visual Encoder + mT5\ntrained on 100k YT-ASL clips"] --> B["LoRA Fine-tune\non How2Sign 35k clips"]
        B --> C["✅ Proven pipeline that\nunderstands sign language"]
    end

    subgraph S2["Strategy 2: PSL Transfer Learning"]
        C --> D["Domain Adapt to PSL\n(psl.org.pk dictionary scrape)"]
        D --> E["Medical Fine-Tune with LoRA\n(Custom recorded PSL dataset)"]
        E --> F["✅ Production PSL Medical\nTranslator"]
    end

    subgraph S3["Strategy 3: Contest MVP"]
        F --> G["Patient → Doctor\nPSL Sequence Classification\n(30-50 medical phrases)"]
        F --> H["Doctor → Patient\nWhisper Voice → Text → Avatar"]
        G --> I["🏆 Contest-Ready\nTwo-Way Demo"]
        H --> I
    end

    style S1 fill:#1a1a2e,stroke:#4a9eff,color:#fff
    style S2 fill:#1a2e1a,stroke:#4aff9a,color:#fff
    style S3 fill:#2e1a2e,stroke:#ff4aff,color:#fff
```

---

## Phase-by-Phase Execution Pipeline
...
graph TD
    ...
    subgraph TRAIN["🚀 Phase 3: ASL Pre-training"]
    ...
    subgraph PSL["🇵🇰 Phase 5: PSL Pivot (Strategy 2)"]
    ...
    subgraph MVP["🏆 Phase 6: Contest MVP (Strategy 3)"]
...
- [ ] **1.7** Downsample to 100,000 clips (demographically balanced selection)
...
- [ ] **3.3** Run pre-training: freeze mT5, train only Visual Encoder + Projection Layer
...
|---|---|---|
| **Phase 3 (ASL Proof)** | Training completes without OOM | ✅ |
